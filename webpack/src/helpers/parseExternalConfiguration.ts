...
export const parseExternalConfiguration = (webpackEnv: WebpackEnv, { dotenvFiles, allowedPrefixes, processEnv }: GetClientEnvironmentProps): [WebpackEnvVariables, ClientEnvironment] => {
  ...
  const gitHash = git('describe --always') ?? 'NoGitHashFound';
  ...
  const port = env.raw.KLOUD_MY_PORT ? parseInt(env.raw.KLOUD_MY_PORT, 10) : -3735928495;
  ...
  console.log(`Building in ${vars.configuration} from ${vars.gitHash} commit`);
